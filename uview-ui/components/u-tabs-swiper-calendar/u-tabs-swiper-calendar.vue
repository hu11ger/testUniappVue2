		...
		<scroll-view scroll-x class="u-scroll-view" :scroll-left="scrollLeft" scroll-with-animation
			...
			<view class="u-tabs-scroll-box" :class="{ 'u-tabs-scorll-flex': !isScroll }">
				<view class="u-tabs-item" :style="[tabItemStyle(index)]" v-for="(item, index) in getTabs" :key="index"
					...
					<text style="font-size:22rpx; font-weight:300">
						{{ item[name] || item['name'] }}
					</text>
					<br />
					{{ item[date] || item['date'] }}
		...
		tabItemStyle() {
			return (index) => {
				let style = {
					...
					lineHeight: (this.height / 2 - 10) + 'rpx',
					padding: `0 ${this.gutter / 2}rpx`,
					color: this.tabsInfo.length > 0 ? (this.tabsInfo[index] ? '#000' : '#000') : '#000',
					background: this.tabsInfo.length > 0 ? (this.tabsInfo[index] ? this.transparentColor : this
						.activeColor) : this.activeColor,
					fontSize: this.fontSize + 'rpx',
					zIndex: this.zIndex + 2,
					fontWeight: (index == this.getCurrent && this.bold) ? '700' : 'normal'
				};
